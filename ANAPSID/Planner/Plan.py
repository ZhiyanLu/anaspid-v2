...
def contactProxy(server, query, queue, buffersize=16384, limit=50):
    ...
    # server is the the url for predicate used in the query
    print("uri is {0}".format(urlparse(server)))
    endpoint = urlparse(server)
    where_condition = query[query.find("{") + 1:query.find("}")]
    print("where_condition is {0}".format(where_condition))
    target = query[query.find("SELECT") + 6:query.find("WHERE")].replace('?', '').split()
    print("target is {0}".format(target))
    if endpoint.scheme == "mongodb":
        # process mongo query

        pass
    elif endpoint.scheme == "file" :
        # process file query
        pass
    ...
    # print("where is {0}".format(where))
    # where_float = float(where)
    # mongo_uri = '{0}://{1}:{2}@{3}{4}'.format(uri.scheme, "root", "31415926", uri.netloc, uri.path,)
...
def createPlan(query, adaptive, wc, buffersize, c, endpointType):
    ...
    print("Physical plan:{0}".format(operatorTree))
    return operatorTree
